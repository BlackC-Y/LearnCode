...
class ngSk2Weight_BbBB():
    ...
    def ToolUi(self, layout=0):
        ...
        if not layout:
            cmds.window(self.Ui, t='ngSk2Weight', rtf=1, mb=1, tlb=1, bgc=QtStyle.backgroundMayaColor)
        ...
        shiboken2.wrapInstance(int(OmUI.MQtUtil.findControl(
            cmds.button(l=u'临时身体模板', w=210, c=lambda *args: self.tempSelectJointUi()))), QPushButton).setStyleSheet(QtStyle.QButtonStyle(height=26))
        shiboken2.wrapInstance(int(OmUI.MQtUtil.findControl(
            cmds.button(l=u'运行', w=210, c=lambda *args: self.doIt()))), QPushButton).setStyleSheet(QtStyle.QButtonStyle(height=26))
        cmds.popupMenu()
        ...
        cmds.frameLayout(cll=1, cl=1, mw=5, l=u'快速创建层', bgc=QtStyle.accentMayaColor())
        ...
        cmds.frameLayout(cll=1, cl=1, mw=5, l=u'复制粘贴权重', bgc=QtStyle.accentMayaColor())
        cmds.columnLayout(cat=('both', 2), rs=2, adj=1)
        shiboken2.wrapInstance(int(OmUI.MQtUtil.findControl(
            cmds.button(l=u'复制点权重', c=lambda *args: self.copyVtxWeight()))), QPushButton).setStyleSheet(QtStyle.QButtonStyle(height=26))
        cmds.rowLayout(nc=2)
        shiboken2.wrapInstance(int(OmUI.MQtUtil.findControl(
            cmds.button(l=u'粘贴选择层', w=93, c=lambda *args: self.pasteVtxWeight(1)))), QPushButton).setStyleSheet(QtStyle.QButtonStyle(height=26))
        shiboken2.wrapInstance(int(OmUI.MQtUtil.findControl(
            cmds.button(l=u'粘贴全部层', w=93, c=lambda *args: self.pasteVtxWeight(0)))), QPushButton).setStyleSheet(QtStyle.QButtonStyle(height=26))
        cmds.setParent('..')
        cmds.setParent('..')
        cmds.setParent('..')
        cmds.setParent('..')
    ...
    def setJointWeight_createLayer(self, skinCluster):
        ...
        layers.prune_weights_filter_threshold = 0.003
