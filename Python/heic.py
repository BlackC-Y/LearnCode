...
def main():
    ...
    for path, dirs, filelist in os.walk(scpath):
            for f in filelist:
                fn = f.split('.')[0]
                os.system(f'ffmpeg -i "{scpath + f}" -crf 10 -pix_fmt yuv420p -sws_flags spline+accurate_rnd+full_chroma_int -color_range 1 -colorspace 5 \
                            -color_primaries 5 -color_trc 6 -f hevc "{f"{sourpath}hvc/{fn}.hvc"}"')
