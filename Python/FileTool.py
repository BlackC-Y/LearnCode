# -*- coding: utf-8 -*-
# Python 3.8
from PySide2.QtCore import Qt, QSize, QMetaObject, QFileInfo, QRect, QPropertyAnimation, QParallelAnimationGroup
from PySide2.QtGui import QIcon, QPainter, QPalette, QColor, QPixmap
...
class FileTool_BlackC_Ui(QMainWindow):
    ...
    def setupUi(self):
        ...
        uiItem['rulesNameCB'] = self.rulesNameCB = QCheckBox(self.reNameTab)
        self.rulesNameCB.setObjectName("rulesNameCB")
        _tempHLayout.addWidget(self.rulesNameCB)
        uiItem['rulesNameTextEdit'] = self.rulesNameTextEdit = QLineEdit(self.reNameTab)
        ...
        _tempHLayout = QHBoxLayout()
        _tempHLayout.setSpacing(20)
        uiItem['prefixCB'] = self.prefixCB = QCheckBox(self.reNameTab)
        self.prefixCB.setObjectName("prefixCB")
        _tempHLayout.addWidget(self.prefixCB)
        uiItem['prefixTextEdit'] = self.prefixTextEdit = QLineEdit(self.reNameTab)
        ...
        _tempHLayout = QHBoxLayout()
        _tempHLayout.setSpacing(20)
        uiItem['suffixCB'] = self.suffixCB = QCheckBox(self.reNameTab)
        self.suffixCB.setObjectName("suffixCB")
        _tempHLayout.addWidget(self.suffixCB)
        uiItem['suffixTextEdit'] = self.suffixTextEdit = QLineEdit(self.reNameTab)
        ...
        _tempHLayout = QHBoxLayout()
        _tempHLayout.setSpacing(20)
        uiItem['extensionCB'] = self.extensionCB = QCheckBox(self.reNameTab)
        self.extensionCB.setObjectName("extensionCB")
        _tempHLayout.addWidget(self.extensionCB)
        uiItem['extensionTextEdit'] = self.extensionTextEdit = QLineEdit(self.reNameTab)
        self.extensionTextEdit.setObjectName("extensionTextEdit")
        _tempHLayout.addWidget(self.extensionTextEdit)
        _tempHLayout.addItem(QSpacerItem(40, 20, QSizePolicy.Expanding, QSizePolicy.Minimum))
        self.reNamePageVLayout.addLayout(_tempHLayout)
        
        _tempVLayout = QVBoxLayout()
        _tempHLayout = QHBoxLayout()
        _tempHLayout.setSpacing(20)
        uiItem['replaceCB'] = self.replaceCB = QCheckBox(self.reNameTab)
        self.replaceCB.setObjectName("replaceCB")
        _tempHLayout.addWidget(self.replaceCB)
        uiItem['replaceSTextEdit'] = self.replaceSTextEdit = QLineEdit(self.reNameTab)
        ...
        _tempHLayout = QHBoxLayout()
        _tempHLayout.setSpacing(20)
        self.replacelabel = QLabel(self.reNameTab)
        _tempHLayout.addWidget(self.replacelabel)
        uiItem['replaceTTextEdit'] = self.replaceTTextEdit = QLineEdit(self.reNameTab)
        ...
        _tempVLayout = QVBoxLayout()
        _tempHLayout = QHBoxLayout()
        uiItem['toLocationCB'] = self.toLocationCB = QCheckBox(self.reNameTab)
        self.toLocationCB.setObjectName("toLocationCB")
        _tempHLayout.addWidget(self.toLocationCB)
        uiItem['startLocationSB'] = self.startLocationSB = QSpinBox(self.reNameTab)
        self.startLocationSB.setObjectName("startLocationSB")
        self.startLocationSB.setMinimumSize(QSize(45, 20))
        self.startLocationSB.setMinimum(1)
        _tempHLayout.addWidget(self.startLocationSB)
        self.TextLoclabelB = QLabel(self.reNameTab)
        ...
        self.Testbutton = QPushButton(self.reNameTab)
        self.Testbutton.setMinimumSize(QSize(80, 26))
        ...
        self.reNamePageVLayout.addItem(QSpacerItem(20, 40, QSizePolicy.Minimum, QSizePolicy.Expanding))
        ...
        self.FiletreeWidget.setMinimumSize(QSize(300, 380))
        self.FiletreeWidget.setRootIsDecorated(False)
        self.FiletreeWidget.setSelectionMode(QAbstractItemView.ExtendedSelection)
        ...
        self.unDobutton = QPushButton(self.Mainwidget)
        self.unDobutton.setObjectName("unDobutton")
        self.unDobutton.setMaximumSize(QSize(35, 35))
        self.buttonVLayout.addWidget(self.unDobutton)
        self.buttonVLayout.addItem(QSpacerItem(20, 40, QSizePolicy.Minimum, QSizePolicy.Expanding))
        
        self.upMoveItembutton = QPushButton(self.Mainwidget)
        self.upMoveItembutton.setObjectName("upMoveItembutton")
        self.upMoveItembutton.setMaximumSize(QSize(35, 35))
        self.buttonVLayout.addWidget(self.upMoveItembutton)
        
        self.deleteItembutton = QPushButton(self.Mainwidget)
        self.deleteItembutton.setObjectName("deleteItembutton")
        self.deleteItembutton.setMaximumSize(QSize(35, 35))
        self.buttonVLayout.addWidget(self.deleteItembutton)

        self.dnMoveItembutton = QPushButton(self.Mainwidget)
        self.dnMoveItembutton.setObjectName("dnMoveItembutton")
        self.dnMoveItembutton.setMaximumSize(QSize(35, 35))
    ...
    def refreshTreeWeight(self):
        if _treeWidgetList := QTreeWidgetItemIterator(self.FiletreeWidget):
    ...
    def deleteSelectFileItem(self):
        _treeWidgetList = QTreeWidgetItemIterator(self.FiletreeWidget)
        index = self.tabWidget.currentIndex()
        items = self.FiletreeWidget.selectedItems()
...
class coustomTreeWidget(QTreeWidget):
    ...
    def addFileTreeItem(self, dforf, path):
        ...
        if dforf:
            _oneItem = QTreeWidgetItem(uiItem['FiletreeWidget'], [filename, filename])
        else:
            KBs = round(os.path.getsize(path) / 1024, 0)     # 命令返回的是字节大小
            size = '%s MB' %KBs / 1024 if KBs > 10000 else '%s KB' %KBs
            _oneItem = QTreeWidgetItem(uiItem['FiletreeWidget'], [filename, time.strftime("%Y-%m-%d %H:%M", time.localtime(os.stat(path).st_mtime)), size])
        _oneItem.setIcon(0, icon)
        _oneItem.setData(0, Qt.UserRole, path)

class coustomVTextLine(QLabel):
...
class FileTool_BlackC():
    ...
    def reNameProc(self):
        _treeWidgetList = QTreeWidgetItemIterator(uiItem['FiletreeWidget'])
        if not _treeWidgetList:
            return
        itemList = [i.value() for i in _treeWidgetList]
        newNameList = [[i.text(1), i.data(0, Qt.UserRole)] for i in itemList]
        unDoData.append([])
        for i, w in zip(newNameList, itemList):
            ...
            w.setText(0, i[0])
            w.setData(0, Qt.UserRole, _newFile)
