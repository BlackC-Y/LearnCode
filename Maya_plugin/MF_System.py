...
def createMF():
    ...
    if cmds.ls('master_MF|MidJoint_loc'):
        ...
        cmds.parent('RootJoint', cmds.spaceLocator(n='_keyInfoLoc'), cmds.spaceLocator(n='_tempMatrixLoc'), 'master_MF')
        cmds.setAttr(cmds.group(cmds.ls("master_MF|*_Surface"), n='MF_Surface_grp') + '.visibility', 0)
        layerGrpN = ['_rotZGrp', '_rotYGrp', '_rotXGrp']
        for i in JointName[1:]:
            _preN = i.rsplit('_', 1)[0]
            _jPos = cmds.xform(i, q=1, t=1, os=1)
            for g in range(len(layerGrpN)):
                _grpN = cmds.group(i, n=_preN + layerGrpN[g]) if g == 0 else cmds.group(_preN + layerGrpN[g-1], n=_preN + layerGrpN[g])
                cmds.setAttr(_grpN + '.rp', _jPos[0], _jPos[1], _jPos[2], e=1)
        posInfo = ''
        for i in JointName:
        ...
        if Rotate:
            ...
            if cmds.checkBox('KeymodecheckBox', q=1, v=1):
                for j in ctrlJoint:
                    if j == 'Center_Joint':
                        continue
                    cmds.setKeyframe('%s.t' % j, '%s_rotZGrp.rz' % j.rsplit('_', 1)[0], '%s_rotYGrp.ry' % j.rsplit('_', 1)[0], '%s_rotXGrp.rx' % j.rsplit('_', 1)[0], t=preFrame)
            twoAngle = [45, 45] if Rotate == 90 else [-45, -45]
            layerGrpN = ['_rotZGrp', '_rotYGrp', '_rotXGrp', '_Joint', '_Joint', '_Joint']
            for a, f in zip(twoAngle, [halfFrame, newFrame]):
                if 'Center_Joint' in ctrlJoint:
                    for j in ctrlJoint:
                        if j == 'Center_Joint':
                            continue
                        cmds.xform('_tempMatrixLoc', m=self.multMatrix(cmds.xform(j, q=1, m=1), _tempAxis, a))
                        _preN = j.rsplit('_', 1)[0]
                        for g, tr in zip(layerGrpN, ['.rz', '.ry', '.rx', '.tx', '.ty', '.tz']):
                            cmds.setAttr(_preN + g + tr, cmds.getAttr('_tempMatrixLoc' + tr))
                else:
                    for j in ctrlJoint:
                        if 'L' in j:
                            cmds.xform('_tempMatrixLoc', m=self.multMatrix(cmds.xform(j, q=1, m=1), _tempAxis, a))
                            ...
                            if _tempAxis == '.rx':
                                cmds.rotate(a, 0, 0, j, r=1, os=1, fo=1)
                            elif _tempAxis == '.ry':
                                cmds.rotate(0, a, 0, j, r=1, os=1, fo=1)
                            elif _tempAxis == '.rz':
                                cmds.rotate(0, 0, a, j, r=1, os=1, fo=1)
                if cmds.checkBox('KeymodecheckBox', q=1, v=1):
                    for j in ctrlJoint:
                        if j == 'Center_Joint':
                            continue
                        cmds.setKeyframe('%s.t' % j, '%s_rotZGrp.rz' % j.rsplit('_', 1)[0], '%s_rotYGrp.ry' % j.rsplit('_', 1)[0], '%s_rotXGrp.rx' % j.rsplit('_', 1)[0], t=f)
                    cmds.setKeyframe('_keyInfoLoc.t', '_keyInfoLoc.r', t=f)
        cmds.select(ctrl, r=1)
        cmds.button('SetVtrlButton', e=1, bgc=[0, 1, 0])
    '''
    def setCtrl__(self, Rotate):
        ctrl = cmds.ls(sl=1)[0]
        if not re.match('\S*_Joint_Ctrl', ctrl):
            return
        midName = ctrl.split('_')[0]
            ...
            for a, f in zip(twoAngle, [halfFrame, newFrame]):
                if 'Center_Joint' in ctrlJoint:
                    for j in ctrlJoint:
                        if j == 'Center_Joint':
                            continue
                        cmds.xform(j, m=self.multMatrix(cmds.xform(j, q=1, m=1), _tempAxis, a))
                else:
                    for j in ctrlJoint:
                        if 'L' in j:
                            cmds.xform(j, m=self.multMatrix(cmds.xform(j, q=1, m=1), _tempAxis, a))
